#!/usr/bin/env python3
"""
Chain-of-Thought Task (1-shot only):
Build/run 1-shot prompts for CIF -> properties using CoT reasoning.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chain-of-Thought task (1-shot only) for CIF -> properties."
    )
    ...
    build.add_argument(
        "--prompts-out",
        type=Path,
        default=Path("results/task_1_cot_llm/1shot/prompts.jsonl"),
        help="Output JSONL with prompts.",
    )
    build.add_argument(
        "--gold-out",
        type=Path,
        default=Path("results/task_1_cot_llm/1shot/gold.jsonl"),
    ...
    run.add_argument(
        "--out-dir",
        type=Path,
        default=Path("results/task_1_cot_llm/1shot"),
    ...
    run.add_argument(
        "--title",
        type=str,
        default="SCALAR CIF->properties (1-shot CoT)",
...
def build_reasoning_gap(args: argparse.Namespace, shots: int = 1) -> int:
...
def main() -> int:
    ...
    if args.command == "build":
        return build_reasoning_gap(args, shots=1)
    if args.command == "run":
