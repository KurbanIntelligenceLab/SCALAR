...
try:
    ...
    warnings.warn(
        "torch_geometric not available. Install with: pip install torch-geometric"
    )
...
R_SPLITS["TRAIN"] = [
    r for r in R_VALUES_ALL if r not in R_SPLITS["ID"] and r not in R_SPLITS["OOD"]
]
...
class NanoparticleDataset(Dataset):
    ...
    def _verify_files(self):
        ...
        for item in self.data_items:
            xyz_path = get_xyz_path(
                self.xyz_base_dir, item["material"], item["r_value"]
            )
        ...
        if missing:
            logger.warning(
                f"Missing {len(missing)} XYZ files in {self.split_name} dataset:"
            )
    ...
    def _compute_stats(self):
        ...
        for prop in self.target_properties:
            logger.info(
                f"    {prop}: mean={self.target_means[prop]:.2f}, std={self.target_stds[prop]:.2f}"
            )
    ...
    def __getitem__(self, idx):
        ...
        # Get XYZ path
        xyz_path = get_xyz_path(self.xyz_base_dir, item["material"], item["r_value"])
    ...
    def denormalize(self, predictions: torch.Tensor) -> torch.Tensor:
        ...
        for i, prop in enumerate(self.target_properties):
            denorm[:, i] = (
                predictions[:, i] * self.target_stds[prop] + self.target_means[prop]
            )
        return denorm
...
class SchNet(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"  hidden_channels={hidden_channels}, num_interactions={num_interactions}"
        )
...
class CGCNN(nn.Module):
    ...
    def __init__(
        ...
        self.convs = nn.ModuleList(
            [CGConv(hidden_channels, num_gaussians) for _ in range(num_layers)]
        )
    ...
    for batch in loader:
        ...
        for i in range(batch.num_graphs):
            ...
            all_materials.append(
                batch.material[i] if hasattr(batch, "material") else "unknown"
            )
...
def discover_xyz_files(
    xyz_base_dir: str, materials: List[str] = None
) -> Dict[str, List[Dict]]:
    """
...
def run_gnn_baselines(
    ...
    if not TORCH_GEOMETRIC_AVAILABLE:
        raise RuntimeError(
            "torch_geometric required. Install with: pip install torch-geometric"
        )
    ...
    test_dataset.set_normalization(
        train_dataset.target_means, train_dataset.target_stds
    )
    ...
    for model_name in models_to_run:
        ...
        try:
            ...
            logger.info(f"{'=' * 70}")
            logger.info(
                f"{'Property':<20} {'ID MAE':>10} {'ID MAPE%':>10} {'OOD MAE':>10} {'OOD MAPE%':>10}"
            )
            logger.info("-" * 70)

            for prop in config.target_properties:
                id_mae = (
                    test_metrics.get("ID", {}).get(prop, {}).get("mae", float("nan"))
                )
                id_mape = (
                    test_metrics.get("ID", {}).get(prop, {}).get("mape", float("nan"))
                )
                ood_mae = (
                    test_metrics.get("OOD", {}).get(prop, {}).get("mae", float("nan"))
                )
                ood_mape = (
                    test_metrics.get("OOD", {}).get(prop, {}).get("mape", float("nan"))
                )

                logger.info(
                    f"{prop:<20} {id_mae:>10.2f} {id_mape:>10.2f} {ood_mae:>10.2f} {ood_mape:>10.2f}"
                )
...
Examples:
    ...
    parser.add_argument(
        "--xyz-dir", "-x", required=True, help="Base directory for XYZ files"
    )
    parser.add_argument(
        "--output", "-o", default="gnn_results", help="Output directory"
    )
    parser.add_argument(
        "--model",
        "-m",
