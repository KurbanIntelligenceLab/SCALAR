#!/usr/bin/env python3
"""Build 3-shot prompts (train R examples) for CIF -> properties."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build 3-shot prompts for CIF -> nanoparticle properties."
    )
    sub = parser.add_subparsers(dest="command", required=True)
    build = sub.add_parser("build", help="Build 3-shot prompts and gold labels.")
    run = sub.add_parser("run", help="Run 3-shot prompts on OpenRouter.")
    ...
    build.add_argument(
        "--prompts-out",
        type=Path,
        default=Path("results/task_1_llm/3shot/prompts.jsonl"),
        help="Output JSONL with prompts.",
    )
    build.add_argument(
        "--gold-out",
        type=Path,
        default=Path("results/task_1_llm/3shot/gold.jsonl"),
    ...
    run.add_argument(
        "--out-dir",
        type=Path,
        default=Path("results/task_1_llm/3shot"),
    ...
    run.add_argument(
        "--title",
        type=str,
        default="SCALAR CIF->properties (3-shot)",
...
def main() -> int:
    ...
    if args.command == "build":
        return build_fewshot_tasks(args, shots=3)
    if args.command == "run":
