#!/usr/bin/env python3
"""
Inverse Retrieval Task (5 candidates):
Given properties -> identify the correct CIF among 5 candidates.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inverse retrieval task: properties -> CIF (5 candidates)."
    )
    ...
    build.add_argument(
        "--prompts-out",
        type=Path,
        default=Path("results/task_inverse_llm/5cand/prompts.jsonl"),
        help="Output JSONL with prompts.",
    )
    build.add_argument(
        "--gold-out",
        type=Path,
        default=Path("results/task_inverse_llm/5cand/gold.jsonl"),
    ...
    run.add_argument(
        "--out-dir",
        type=Path,
        default=Path("results/task_inverse_llm/5cand"),
    ...
    run.add_argument(
        "--title",
        type=str,
        default="SCALAR inverse retrieval (5 candidates)",
